...
Nota: Agregar un timeout de 300ms genera errores al enviar el tiempo de encendido.
...
int main(void)
{
   ...
   // Crear tarea 1
   xTaskCreate(vTarea1,                  // Función de la tarea a ejecutar
   ...
   // Crear tarea 2
   xTaskCreate(vTarea2,                  // Función de la tarea a ejecutar
   ...
   // Crear tarea 3
   xTaskCreate(vTarea3,                  // Función de la tarea a ejecutar
...
void vTarea1(void *p)
{
   while (1)
   {
      ...
      xQueueSend(xCola, &tiempo, portMAX_DELAY); // Espera a max delay
      // if (xQueueSend(xCola, &tiempo, 300 / portTICK_PERIOD_MS) != pdPASS) // timeout de 300 ms
      // {
...
void vTarea3(void *p)
{
   while (1)
   {
      ...
      uint32_t tiempo;
      if (xQueueReceive(xCola, &tiempo, pdMS_TO_TICKS(300)) == pdTRUE)
      {
